...
public class ElementHelper {
    ...
    public void scrollInsideAndClick(By scrollElement, By locator) {
        try {
            wait.until(ExpectedConditions.presenceOfElementLocated(locator));
        } catch (Exception e) {
            scrollInside(scrollElement, locator).get(0);
        }
    }
    ...
    public List<WebElement> scrollInside(By scrollElement, By by) {
        int i = 0;
        String previusPageSource = ".";
        while (checkElemenEnabled(by) && i < 5 && checkEndOfPage(previusPageSource)) {
            i++;
    ...
    public void scrollDown(By scrollElement) {
        int startX = driver.findElement(scrollElement).getSize().getWidth() / 2;
        int startY = driver.findElement(scrollElement).getSize().getHeight() / 2;
        int endx = driver.findElement(scrollElement).getSize().getWidth() / 2;
        int endY = (int) (driver.findElement(scrollElement).getSize().getHeight() * 0.2);
